...
function navReducer(state: NavState, action: NavAction): NavState {
  switch (action.type) {
    ...
    case "RESET":
      return {
        stack: action.entries.length ? action.entries : [initial.stack[0]],
      };
    case "UPDATE_TOP_STATE": {
      ...
      const newTop = {
        ...top,
        state: { ...(top.state ?? {}), ...action.statePatch },
      };
...
export function NavigationProvider({
  ...
  const value: NavContextValue = {
    stack: state.stack,
    current: state.stack[state.stack.length - 1] ?? initial.stack[0],
    push: (name, params = null) =>
      dispatch({ type: "PUSH", entry: { name, params } }),
    replace: (name, params = null) =>
      dispatch({ type: "REPLACE", entry: { name, params } }),
    back: (steps) => dispatch({ type: "BACK", steps }),
    reset: (entries) => dispatch({ type: "RESET", entries }),
    updateTopState: (patch) =>
      dispatch({ type: "UPDATE_TOP_STATE", statePatch: patch }),
  };
